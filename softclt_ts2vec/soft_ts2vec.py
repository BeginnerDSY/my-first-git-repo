...
#     def fit(self, train_data, train_labels, test_data, test_labels, soft_labels, run_dir, n_epochs=None, n_iters=None, verbose=False):
    def fit(self, train_data, train_labels, test_data, test_labels, soft_labels, run_dir,train_phase=None, n_epochs=None, n_iters=None, verbose=False):
...
#             train_dataset = custom_dataset(train_data)
#         else:
#             train_dataset = custom_dataset(train_data, train_phase)
...
#             phase = phase.to(self.device)python -m py_compile soft_ts2vec.py utils.py
        train_data = train_data[~np.isnan(train_data).all(axis=2).all(axis=1)]
        ...
        while True:
            ...
            for batch in train_loader:
                ...
                if self.max_train_length is not None and x.size(1) > self.max_train_length:
                    ...
                    x = x[:, window_offset : window_offset + self.max_train_length]
                    if phase is not None:
                        phase = phase[:, window_offset : window_offset + self.max_train_length]
